...
<div style="margin-top:20px;font-weight:bold">PRODUCT / CREATE / GRAPHICS CARD</div>
<h2 class="text-center">CREATE NEW PRODUCT (GRAPHICS CARD)</h1>
    <div class="dropdown">
    ...
    <form action="{{URL::to('admin/save_product')}}" method="post" enctype="multipart/form-data">
        ...
        <ul>
            <li> <div class="form-group">
                CPU
                <select name="cpu" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($cpu as $key=>$cp)
                    <option value="{{$cp->id}}"> {{$cp->cpu_name}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Video ouput
                <select name="video_output" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($video_output as $key=>$vi_out)
                    <option value="{{$vi_out->id}}"> {{$vi_out->video_output}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
            ...
            <li> <div class="form-group">
                ...
                <select name="memory_size" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($memory_size as $key=>$s_mem)
                    <option value="{{$s_mem->id}}"> {{$s_mem->memory_size}}</option>
                    @endforeach
                </select>
            </div></li>
        </ul>
