...
<div style="margin-top:20px;font-weight:bold">PRODUCT / CREATE / MONITOR</div>
<h2 class="text-center">CREATE NEW PRODUCT (MONITOR)</h1>
    <div class="dropdown">
    ...
    <form action="{{URL::to('admin/save_product')}}" method="post" enctype="multipart/form-data">
        ...
        <ul>
            <li> <div class="form-group">
                Resolution
                <select name="resolution" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($resolution as $key=>$resol)
                    <option value="{{$resol->id}}"> {{$resol->resolution_value}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Size
                <select name="monitor_size" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($monitor_size as $key=>$mo_size)
                    <option value="{{$mo_size->id}}"> {{$mo_size->monitor_size}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Refresh Rate
                <select name="refresh_rate" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($refresh_rate as $key=>$re_rate)
                    <option value="{{$re_rate->id}}"> {{$re_rate->refresh_rate_value}}</option>
                    @endforeach
                </select>
            </div></li>
        </ul>
