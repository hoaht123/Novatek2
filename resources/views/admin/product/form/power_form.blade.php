...
<div style="margin-top:20px;font-weight:bold">PRODUCT / CREATE / POWER (PSU)</div>
<h2 class="text-center">CREATE NEW PRODUCT (POWER PSU)</h1>
    <div class="dropdown">
    ...
    <form action="{{URL::to('admin/save_product')}}" method="post" enctype="multipart/form-data">
        ...
        <ul>
            <li> <div class="form-group">
                Power
                <select name="power" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($power as $key=>$pow)
                    <option value="{{$pow->id}}"> {{$pow->power_value}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Efficiency
                <select name="efficiency" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($efficiency as $key=>$eff)
                    <option value="{{$eff->id}}"> {{$eff->efficiency_value}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Fan size
                <select name="fan_size" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($fan_size as $key=>$fan_s)
                    <option value="{{$fan_s->id}}"> {{$fan_s->fan_size}}</option>
                    @endforeach
                </select>
            </div></li>
        </ul>
