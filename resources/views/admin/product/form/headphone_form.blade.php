...
<div style="margin-top:20px;font-weight:bold">PRODUCT / CREATE / HEADPHONE</div>
<h2 class="text-center">CREATE NEW PRODUCT (HEADPHONE)</h1>
    <div class="dropdown">
        ...
        <ul>
            <li> <div class="form-group">
                ...
                <select name="headphone_type" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($headphone_type as $key=>$hp_type)
                    <option value="{{$hp_type->id}}"> {{$hp_type->headphone_type}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Wireless
                <select name="wireless" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($wireless as $key=>$wire)
                    <option value="{{$wire->id}}"> {{$wire->isWireless}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                Micro
                <select name="micro" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($micro as $key=>$micr)
                    <option value="{{$micr->id}}"> {{$micr->isMicro}}</option> 
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
