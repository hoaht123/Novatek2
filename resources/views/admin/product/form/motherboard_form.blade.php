...
<div style="margin-top:20px;font-weight:bold">PRODUCT / CREATE / MOTHERBOARD</div>
<h2 class="text-center">CREATE NEW PRODUCT (MOTHERBOARD)</h1>
    <div class="dropdown">
    ...
    <form action="{{URL::to('admin/save_product')}}" method="post" enctype="multipart/form-data">
        ...
        <ul>
            <li> <div class="form-group">
                Size form
                <select name="size_form" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($size_form as $key=>$size_mb)
                    <option value="{{$size_mb->id}}"> {{$size_mb->size_form_name}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
                ...
                <select name="socket_type" class="form-control"style="width:200px">
                    <option value="">-----Choose-----</option>
                    @foreach($socket_type as $key=>$sk_mb)
                    <option value="{{$sk_mb->id}}"> {{$sk_mb->socket_name}}</option>
                    @endforeach
                </select>
            </div></li>
            <li> <div class="form-group">
